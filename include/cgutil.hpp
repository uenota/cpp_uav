...
* half of the outer product of vector \f$ \vec{p_1p_2} \f$ and \f$ \vec{p_1p_3} \f$.\n
...
*           \end{array} 
*       \right| \\
*           & = & p_1.x(p_2.y - p_3.y) - p_1.y(p_2.x - p_3.x) - (p_2.x\times p_3.y - p_2.y\times p_3.x)
*   \f}
*/
inline double calc_signed_area_triangle(const geometry_msgs::Point &p1,
...
*/
double calc_vertex_angle(const geometry_msgs::Point &p1,
                         const geometry_msgs::Point &p2,
                         const geometry_msgs::Point &p3)
{
...
*/
double calc_horizontal_angle(const geometry_msgs::Point &p1, const geometry_msgs::Point &p2)
{
    geometry_msgs::Point p3;
    p3.x = p1.x + 1.0;
    p3.y = p1.y;
    return calc_vertex_angle(p1, p2, p3);
}
...
*/
double calc_distance(const std::array<geometry_msgs::Point, 2> &edge, const geometry_msgs::Point &vertex)
{
    ...
    double alpha, beta;
    alpha = calc_vertex_angle(point_a, point_b, vertex);
    beta = calc_vertex_angle(point_b, point_a, vertex);
...
*/
std::vector<geometry_msgs::Point> graham_scan(std::vector<geometry_msgs::Point> points)
{
    // convex hull
    ...
    // sort by an angle between a segment composed of min_y_point and pj (in a set of points) and horizontal line
    // in an ascending order
    std::stable_sort(points.begin(), points.end(),
              [&](const geometry_msgs::Point &p1, const geometry_msgs::Point &p2)
                  { return calc_horizontal_angle(min_y_point, p1) < calc_horizontal_angle(min_y_point, p2); });
    ...
    for (const auto &point : points)
    {
        ...
        {
            if(calc_signed_area_triangle(convex_hull.at(i-1), convex_hull.at(i), point) >= 0)
            {
                break;
            }
